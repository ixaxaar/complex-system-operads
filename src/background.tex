...
\subsubsection{Categorical probability and stochastic mechanics}
Our framework draws upon two distinct mathematical traditions. First, categorical probability theory provides the rigorous foundations for stochastic mappings. The category $\mathbf{Stoch}$, consisting of measurable spaces and Markov kernels, was formalized by \citet{giry1982categorical} and further developed by \citet{fritz2020synthetic} through Markov categories. These frameworks treat probability abstractly but focus on static mappings rather than the causal, temporal dynamics inherent to complex systems.

Second, the rigorous study of phase transitions in statistical mechanics provides the tools for quantifying emergence. Central to this is Dobrushin's uniqueness condition \citep{dobrushin1968description}, which established criteria for the decay of correlations in lattice systems. When correlations decay sufficiently fast, the system exhibits a unique Gibbs measure and no phase transition occurs. Conversely, the violation of this condition allows for long-range order and the emergence of distinct macroscopic phases. Our work adapts these concepts: coupling strength, correlation decay, and uniqueness into the operadic setting.
