...
% Define colors
\definecolor{wdcolor}{RGB}{70, 130, 180}    % Steel blue for wiring diagrams
\definecolor{tempcolor}{RGB}{220, 20, 60}   % Crimson for temporal processes
\definecolor{enrichcolor}{RGB}{50, 205, 50} % Lime green for enrichment
\definecolor{compcolor}{RGB}{255, 140, 0}   % Dark orange for composition
...
\begin{scope}[shift={(-6, -2.5)}]
    ...
    % Sample space
    \draw[thick, tempcolor] (-1.5, 0) ellipse (1.2 and 0.8);
    \node[tempcolor] at (0, 0.3) {$\Omega$};
    \node[tempcolor, font=\tiny] at (0, 0) {(execution paths)};

    % Trajectories
    \draw[tempcolor, thick] (-1, -0.3) .. controls (-0.5, 0.1) and (0, 0.2) .. (0.5, -0.1);
    \draw[tempcolor, thick] (-0.8, 0.4) .. controls (-0.3, 0.1) and (0.2, -0.2) .. (0.7, 0.3);
    \draw[tempcolor, thick] (-1.2, 0) .. controls (-0.6, -0.3) and (0.1, 0.4) .. (0.9, 0.1);

    % Filtration
    \node[below, tempcolor] at (0, -1.2) {$\mathcal{F}_t \subseteq \mathcal{F}_{t'} \text{ for } t \leq t'$};
    \node[below, tempcolor, font=\tiny] at (0, -1.5) {(information accumulation)};

    % Measure
    \node[below, tempcolor] at (0, -1.8) {$\mathbb{P}: \mathcal{F} \to [0,1]$};
\end{scope}
